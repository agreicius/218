...
\begin{document}
	...
	\begin{enumerate}
		\item Write the following sets using interval notation. Your answer should be an interval or a union of intervals.
		\begin{enumerate}
		...
		\end{tikzpicture}
		\begin{solution}
			Not a function. The $y$-axis is a vertical line that intersects the graph of the function at 2 points.
		\end{solution}
	...
	\end{tikzpicture}
	\begin{solution}
		Not a function. The $y$-axis is a vertical line that intersects the graph at two points.
	\end{solution}
	...
	\end{enumerate}
		...
		\begin{itemize}
			...
			\item For each function, find the value of $f(0), f(1), f(-1)$ or state that it does not exist.
	...
	\section{Linear and Quadratic Functions}
		...
		\begin{enumerate}
			...
			\begin{solution}
				...
				Since the slope $m=-\dfrac{1}{2}<0$, the function is decreasing, and hence, has no minimum/maximum.
				
				To find the $x$ for which the function is positive/negative, we need to solve the inequalities $f(x)>0$ and $f(x)<0$. Note that it is enough to solve on of them.
				$$f(x)>0$$
				$$-\dfrac{1}{2}x+2>0$$
				$$-\dfrac{1}{2}x>-2$$
				$$x<4.$$
		...
		\begin{enumerate}
			...
			\begin{solution}
				...
				Hence, $x_1=\dfrac{-3-7}{2}=-5$ and $x_2=\dfrac{-3+7}{2}=2$. However, $x$ cannot be $2$ by the conditions of the problem, so the only solution to the equation is $x=-5$.
			\end{solution}
		\end{enumerate}
		...
		\begin{enumerate}
			...
			\begin{solution}
				...
				To find the $x$-intercept, we solve the equation $-x^2-\dfrac{9}{4}$, which is equivalent to $x^2=-\dfrac{9}{4}$. But this equation has no solutions and therefore there are no $x$-intercepts. To find the $y$-intercept, we set $x=0$ i.e. $-0^2-\dfrac{9}{4}=-\dfrac{9}{4}$, so the $y$-intercept is $(0,-\dfrac{9}{4})$.
		...
		\begin{solution}
			The equation of a nonvertical line passing through two points $(x_1,y_1)$ and $(x_2,y_2)$ is $y-y_0=m(x-x_0)$, where the slope $m=\dfrac{y_2-y_1}{x_2-x_1}$. Note that it does not matter which point is first and which is second - they form the same line. For the given line, the slope is $m=\dfrac{2-0}{1-(-1)}=1$, so the equation of the line is $y-2=1(x-1)$, or $y=x+1$.
		\end{solution}
	...
	\begin{enumerate}
		...
		\begin{solution}
			...
			$$f_3(4)=2\cdot 4^{-5/2}=2\cdot (2^2)^{5/2}=2\cdot 2^5=64$$
		...
		\begin{enumerate}
			...
			\begin{solution}
				$\dfrac{(x^3y)^{-1}(8^{-1}x^4y^3)}{(2x^2y^{-3})^{-2}}=\dfrac{x^{-3}y^{-1}8^{-1}x^4y^3}{2^{-2}x^{-4}y^6}=\dfrac{xy^28^{-1}}{2^{-2}x^{-4}y^6}=\dfrac{xx^42^2}{8y^{6-2}}=\dfrac{x^5}{2y^2}.$
			\end{solution}
		\end{enumerate}
		...
		\begin{enumerate}
			...
			\begin{solution}
				\begin{align*}
					&(x-2)(x^2+2x+4)-(x-2)^3-5x^2\\
					&=(x^3-2^3)-(x^3-6x^2+12x-8)-5x^2\\
					&=x^3-8-x^3+6x^2-12x+8-5x^2\\
					&=x^2-12x
				\end{align*}
				The degree of the polynomial is 2, the leading coefficient is 1, the leading term is $6x^2$ and the constant term is $0$.
			\end{solution}
		...
		\item Determine the values of $a,b,c$ so that the polynomials $f(x)$ and $g(x)$ are equal.
		...
		\item Classify the following functions as one of the following: constant functions, power functions, polynomial functions that are not constant, rational functions that are not polynomial, algebraic functions that are none of the previous types.
		\begin{reading}
			Section: 1.4
		\end{reading} 
		\begin{enumerate}
		...
		\begin{enumerate}
			...
			\begin{solution}
				This is a rational function that is defined for all real numbers such that $2x^2-3x+1=0$. The solutions to this equation are $x=1/2$ and $x=1$ and hence, the domain of the function is $\mathbb{R} \setminus \{1,\dfrac{1}{2}\}$.
			...
			\begin{solution}
				...
				To find the zeroes, we need to solve $\dfrac{x^2+1}{x^3-2x+1}$, i.e., $x^2+1=0$. This equation has no real solutions and hence, the function has no zeroes.
			\end{solution}
