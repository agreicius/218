...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
				...
				are equal functions.
				\begin{solution}
					False. The domain of $f(x)$ is $\mathbb{R}\setminus\{2\}$ and the domain of $g(x)$ is $\mathbb{R}$.
				\end{solution}
				...
				is $[2,5)\cup(5,\infty)$.
				\begin{solution}
					True. The numerator is a square root function, which is defined for all $x$ such that $x-2\geq0\iff x\geq 2$. However, the denominator cannot be 0, so we have $x-5\neq0 \iff x\neq 5$. Thus, the domain is $[2,5)\cup(5,\infty)$.
				\end{solution}
				\item The function $f(x)=x^{5/2}-x^{3/2}$ is a power function.
				\begin{solution}
					False. $f(x)$ cannot be written in the form $Ax^k$, for $A\in\mathbb{R}$ and $k$ a rational number.
				\end{solution}
				\item The function $f(x)=\dfrac{x^3+x^2}{x}$ is a polynomial function.
				\begin{solution}
					False. This is a rational function, but it is not polynomial (even though the expression simplifies to $x^2+x$ for $x\neq0$).
				\end{solution}
				\item The function $f(x)=x^{5/3}$ is defined for every real number $x$.
				\begin{solution}
					True. Any power function of the form $f(x)=Ax^k$, where $k$ is a positive rational number with an odd denominator, has domain $\mathbb{R}$.
				\end{solution}
				
				\setcounter{enumi}{3}
				\item The function $f(x)=x^{-2/3}$ is defined at $x=0$.
				\begin{solution}
					False. The power functions of the form $f(x)=Ax^k$, where $k$ is a negative rational number, have domain $\mathbb{R}\setminus\{0\}$.
				\end{solution}
				
				\item Every polynomial function is an algebraic function.
				\begin{solution}
					True. The polynomial functions are a subclass of the algebraic functions.
				\end{solution}
				\item Every algebraic function is a polynomial function. 
				\begin{solution}
					False. For example, the function $f(x)=\sqrt{x}$ is algebraic but not polynomial.
				\end{solution}
				...
				\begin{solution}
					True. $x^4-16=0\iff (x^2-4)(x^2+4)=0\iff (x-2)(x+2)(x^2+4)=0\iff x=2,x=-2$ (since $x^2+4=0$ has no real solutions). Thus, the only real zeroes of the polynomial are -2 and 2.
				\end{solution}
				...
				$$
				\begin{solution}
					False. The function is not defined for negative numbers.
				\end{solution}
		\end{enumerate}
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item Determine which of the following statements are true and which are false. Justify your reasoning.
			\begin{enumerate}
